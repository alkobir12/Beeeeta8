...
export const renderDocumentTemplate = (templateHtml, payload = {}, workshop = {}) => {
  ...
  const titles = { invoice: 'فاتورة مبيعات', diagnosis: 'تقرير تشخيص', quote: 'عرض سعر' };
  const taxAmount = Number(settings?.totals?.tax || payload.tax || 0);
  ...
  const values = {
    ...
    SUBTOTAL: money(subtotal), DISCOUNT: money(discount), TAX: money(taxAmount), TOTAL: money(total + taxAmount), PAID: money(paid), REMAINING: money(total + taxAmount - paid), NOTES: settings.notes || payload.notes || '—', AMOUNT_WORDS: `فقط ${money(total + taxAmount)} لا غير`, SEAL_CODE: settings.seal_code || '—', CUSTOMER_APPROVAL_STAMP: approvalStamp(approvals.customer, 'اعتماد العميل'), WORKSHOP_APPROVAL_STAMP: approvalStamp(approvals.workshop, 'اعتماد الورشة'), DOCUMENT_TITLE: titles[payload.doc_type || payload.docType || settings.doc_type] || 'مستند', TAX_ROW: taxAmount ? `<div><span>الضريبة</span><b>${money(taxAmount)}</b></div>` : '', BARCODE_VALUE: documentNumber,
  };
