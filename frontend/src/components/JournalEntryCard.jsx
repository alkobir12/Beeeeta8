...
export default function JournalEntryCard({
  ...
  // ترتيب العنوان بدون تكرار: الطرف ← المركبة ← الوصف ← نوع القيد
  const { title, subtitle, descriptionUsedInHeader } = useMemo(() => {
    if (partyName) {
      return {
        title: partyName,
        subtitle: vehicleLabel ? `مركبة: ${vehicleLabel}` : description,
        descriptionUsedInHeader: !vehicleLabel && Boolean(description),
      };
    }
    if (vehicleLabel) {
      return { title: `مركبة: ${vehicleLabel}`, subtitle: description, descriptionUsedInHeader: Boolean(description) };
    }
    if (description) {
      return { title: description, subtitle: '', descriptionUsedInHeader: true };
    }
    return { title: typeLabel, subtitle: '', descriptionUsedInHeader: false };
  }, [partyName, vehicleLabel, description, typeLabel]);

  const showTypePill = title !== typeLabel;

  // إخفاء بنود البيانات الوصفية التقنية (صفرية الطرفين أو بأسماء غير عربية)
  const displayLines = useMemo(() => {
    const lines = Array.isArray(entry?.lines) ? entry.lines : [];
    return lines
      .filter((l) => (Number(l?.debit) || 0) > 0 || (Number(l?.credit) || 0) > 0)
      .map((l) => ({ ...l, display_name: cleanAccountName(l?.account_name) || 'حساب' }));
  }, [entry?.lines]);

  const totalDebit = displayLines.reduce((sum, l) => sum + (Number(l?.debit) || 0), 0);
  const totalCredit = displayLines.reduce((sum, l) => sum + (Number(l?.credit) || 0), 0);
  ...
  return (
    ...
    >
      ...
      <section className="p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h2 className="line-clamp-2 text-[16px] font-black leading-snug text-zinc-950" data-testid={`journal-card-title-${cardId}`}>
              {title}
              {partyName && partyRole ? (
        ...
        <div className="mt-2 flex flex-wrap gap-1.5" data-testid={`journal-card-tags-${cardId}`}>
          {showTypePill ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-zinc-200">
              <table className="w-full text-[11px]" data-testid={`journal-card-lines-table-${cardId}`}>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {displayLines.length ? displayLines.map((line, idx) => (
      ...
      <div className="relative flex items-center gap-2 border-t border-zinc-200 bg-white p-3 rounded-b-[24px]" data-testid={`journal-card-actions-${cardId}`}>
        <button
          type="button"
          className="flex min-h-[48px] flex-1 items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-[13px] font-black text-zinc-800 transition active:scale-[0.99]"
