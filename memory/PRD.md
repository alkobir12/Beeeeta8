...
## CHANGELOG — 2026-07-03 · توحيد محرك النوايا + شراء من مورد (المرحلة 1)
**مختبَر 100% (18/18 اختبار + 3 اختبارات API حرفية)**
- 🔴 **جذر P0 (السبب الحقيقي للـ 4 مسودات)**: المقسِّم النصي القديم في `power_mode._SPLIT_RE` كان يقطع على النقطة `1300.` → يُنتج 4 أجزاء → 4 مسودات. تم **تعطيل `extract_commands`** ليُعيد الرسالة كأمر واحد، وحُوِّل `_SPLIT_RE` إلى وضع legacy diagnostic فقط.
...
- 🛒 **نية `create_purchase` جديدة** بمخطط JSON صارم: `{supplier:{name,is_new,id}, items[], payment_method, vat:{mode,rate}, assumptions[], missing[]}`. LLM prompt مُوسَّع بمثالين حرفيين + قاعدة "مسودة واحدة مهما تعدّدت البنود".
- 🏦 **التوجيه المحاسبي (Decimal، توازن صارم عبر AccountingEngine)**:
   - نقدي → مدين: مخزون قطع غيار (007) / دائن: النقد (003)
   - تحويل → مدين: 007 / دائن: البنك (004)
   - آجل → مدين: 007 / دائن: الموردون (2101 — بربط اسم المورد)
   - VAT excluded/included → بند VAT مدخلات (0451 fallback) يُضاف لجانب المدين + الإجمالي للجانب الدائن.
- 🏷️ **الافتراضات موسومة بدون تكرار**: `⚠️ افترضت: نقدي` (عند غياب طريقة الدفع) · `⚠️ افترضت: بدون ضريبة` (عند غياب حقل VAT) · `🆕 مورد جديد: X (سيُنشأ بالإضافة)` · `🔗 طابقت البند: Y`.
- 📦 **echo-back مضغوط في الشات**: النوع + الطرف + الإجمالي + جدول البنود (اسم × سعر × كمية) + القيد المحاسبي + الافتراضات + شارة الحوكمة "لن يُثبَّت أي قيد مالي دون اعتماد بشري مختلف".
- 🔄 **best-effort inventory update عند الاعتماد**: `_upsert_parts_from_purchase` يزيد كمية القطع المطابقة (`matched_id`) أو يُنشئ صنفاً جديداً — لا يوقف الترحيل عند الفشل.
- 🧪 **حزمة اختبارات جديدة** `/app/backend/tests/test_purchase_intent_phase1.py`: 18/18 (توحيد المقسِّم + VAT split + resolver + confirm_text + AccountingEngine balance + end-to-end via LLM mock).
- 🔧 **إصلاحات مسار**: `create_purchase` مضافة إلى `RISKY_ACTIONS`/`_RESOLVE_TARGET_ACTIONS`/`_ACTION_TO_RUNTIME` (=`purchase`). `VALID_ACTIONS` في runtime تشمل `purchase`. أفعال الشراء (اشتري/شراء/شريت/purchase/buy) مُضافة إلى `_ACTION_VERB_RE` لالتقاط الرسائل بدون `/power`.
- ✅ **اختبار API حرفي**: `اشتري من راكان قلب مستوبيشي L200 بسعر 1300. نقدي` ينتج **مسودة واحدة** (لا 4)، القيد متوازن (1300 مدين 007 / 1300 دائن 003)، status: `pending_approval` — يظهر في «اعتمادات كاترينا».
