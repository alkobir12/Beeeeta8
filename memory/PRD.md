...
- **6 أسطح رفع مُعالَجة** (الماسح أبلغ عن 5؛ وكيل الاختبار كشف **سطحين إضافيين** في iteration_380 وتم إصلاحهما):
  1. ملفات المركبات `server.py` · 2. `routes_vehicle_files.py` (راوتر غير مُركَّب) · 3. أدلة تدقيق المالية `routes_finance_bot.py` · 4. قوالب المستندات `routes_document_templates.py` · 5. **راوتر القوالب القديم `routes_templates.py:136`** (كان بلا حد حجم ولا تحقق محتوى) · 6. **إيصالات سداد العمليات `routes_extended.py:167`** (دليل مالي كان على قرص مؤقت).
  - السطح السابع `routes_references.py` صُنِّف **معالجة مؤقتة**: `tempfile.TemporaryDirectory` + اسم `uuid4` + قراءة كل الصفحات داخل السياق ⇒ تنظيف مضمون في النجاح والاستثناء.
- **`PERSISTENT_LOCAL_UPLOADS_REMAINING = 0`** · `SAFE_TEMP_PROCESSING_PATHS = 1` · `TOKEN_IN_FILE_URL = NO` (خلفية وواجهة).
- **طبقة تخزين جديدة** `backend/core/object_storage.py`: مفاتيح يولّدها الخادم (اسم ملف المستخدم لا يصل للمسار مطلقاً) · حد 25MB · قوائم سماح لكل سطح · تحقق محتوى `magic_verified` / `container_verified` / `text_heuristic` — **ترويسة المتصفح ليست كافية وحدها في أي موضع** · `build_named_object_key` يحفظ عقد رابط الإيصالات دون أي تغيير مخطط.
- **تسليم الملفات في المتصفح**: مكوّن جديد `AuthenticatedFileImage.jsx` — `fetch` بالكوكي ثم blob object URL مع `URL.revokeObjectURL` وحالات تحميل/خطأ. لا توكن في أي رابط صورة أو تنزيل.
- **P0 إصلاح الـReset**: `backend/core/destructive_guard.py` **fail-closed** بـ8 شروط (راية تمكين مخصّصة · تصريح قاعدة البيانات = `DB_NAME` · دور مدير · عبارة `confirm=DELETE_ALL` · ترويسة تأكيد مزدوج مشتقّة من الخادم لكل إجراء · correlation id · سجل تدقيق). الراية **غير مضبوطة** ⇒ كل مسار هدّام **مرفوض الآن**. مُركَّب قبل فحوص الصلاحيات في `routes_extended.py` (DELETE /operations) و`server.py` (`/admin/reset-inventory` الذي كان **بلا أي مصادقة**).
- **الحذف**: التخزين لا يوفّر عملية حذف ⇒ حذف منطقي فقط (`is_deleted`). أدلة التدقيق المالي **retention-first بلا حذف فيزيائي**. بند متبقٍ مُسجَّل: `P1-OBJECT-STORAGE-PHYSICAL-DELETION / RETENTION-POLICY`.
- **سكربت ترحيل الملفات القديمة** `/app/scripts/migrate_local_uploads_to_object_storage.py`: dry-run افتراضي · `--apply` وحده **مرفوض** · لا يحذف الأصل · لا يستبدل كائناً موجوداً (CONFLICT) · **لم يُنفَّذ**. نتائج التشغيل الجاف: discovered=28 valid=12 invalid=16 conflicts=0 total_bytes=21,605,441 orphan_files=17 missing_db_references=3.
- **ملفات قديمة ما زالت على قرص العُقدة وستضيع مع أول نشر** (قراءة فقط، لم يُنقل/يُحذف شيء): `uploads/vehicles` 10/13,780,969 · `finance_audit_evidence` 9/37,014 · `operation_payment_receipts` 6/24,860 · `custom_templates` 3/8,197,309 — الإجمالي **28 ملفاً / 22,039,852 بايت**.
- **الاختبار**: `test_p1_sec_upload_and_reset_guard.py` + `test_p1_sec_upload_functional.py` = **57/57**. مصفوفة الرفض حياً على المعاينة (403 `destructive_operations_disabled` للمسارين، وعدد العمليات لم يتغير). أسماء ملفات عدائية على مسار الإيصالات ⇒ 404 بلا أي بايت. `test_journal_pagination_iter361` + `test_ar_ssot` = 20/20 (صفر انحدار مالي). `/api/health` 200 · صفر HTTP 500 · لا تسريب `_id`.
- **التحقق المستقل**: iteration_380 (كشف السطحين) ← iteration_381 (**0 critical · 0 frontend issues · retest_needed=false**).
- **الالتزام**: `DB_MUTATION=NO` · `FINANCIAL_MUTATION=NO` · `ACCOUNTING_ENGINE_CHANGED=NO` · `ENV_CHANGED=NO` (أُضيف `backend/.env.example` بأسماء متغيرين غير سرّيين فقط) · `SECRETS_CHANGED=NO` · `DEPLOY=NO` · `RESET_EXECUTED=NO` · `SEED_EXECUTED=NO` · `FILES_MIGRATED_NOW=0`.
- **بند أمني جديد للـbacklog**: `routes_finance.py:5583` و`:5750` ترفعان `410` كأول تعليمة لكنهما تحتويان كوداً هدّاماً **غير قابل للوصول** تحته — أي تعديل مستقبلي يحذف الـ`raise` يُسلّحه فوراً.
- التقرير الكامل: `/app/memory/discovery/P1_SEC_UPLOAD_AND_RESET_HARDENING_FINAL_REPORT.md` · التقارير: `/app/test_reports/iteration_380.json`، `iteration_381.json`.


## جلسة 2026-06 (فرع جديد) — DEPLOYMENT READINESS HEALTH CHECK ✅ PASS (بلا نشر · بلا تعديل بيانات)
- **الفحص الأول = FAIL بعائق واحد**: `backend/seed_database.py` كان يحتوي `delete_many({})` على `services` (582) و`technicians` (636) و`workshop_profile` (657) — تنفيذ عرضي أثناء نشر/صيانة يمسح البيانات، وقاعدة الإنتاج والمعاينة **واحدة**.
- **الإصلاح (كود فقط — السكربت لم يُنفَّذ إطلاقاً)**: `from pymongo import UpdateOne` (س9)؛ services: إزالة تكرار بالاسم إلى `unique_services` ثم `bulk_write([UpdateOne({"name":…},{"$setOnInsert":…},upsert=True)], ordered=False)`؛ technicians: upsert على `phone`؛ workshop_profile: `update_one({"id":"workshop_profile"},{"$setOnInsert":…},upsert=True)`. `grep -nE "delete_many|insert_many|\.drop\(|drop_database"` ⇒ **0 نتائج**.
- **الفحص الثاني = PASS** · `findings: []` · `destructive_db_startup_confirmed: false` · compilation/env/CORS/supervisor/ports كلها ✅.
- **التحقق (iteration_379، READ-ONLY) = backend 100%**: `$setOnInsert` في كل المواضع ⇒ لا يُعدَّل مستند قائم ولا يُنتج تكراراً؛ `/api/health` 200؛ بدء تشغيل نظيف؛ login 200؛ `/api/auth/me` 200؛ قراءات تمثيلية 200؛ HTTP 500 = 0؛ لا تسريب `_id`. السكربت **لم يُنفَّذ**، صفر تعديل بيانات. نقطة الاستدعاء الوحيدة `/app/backend_test_new_apis.py:345` (لا startup ولا route).
- **تصحيح ذاتي**: ادّعائي السابق في هذه الجلسة بوجود `PREVIEW_CONFIGURATION_DRIFT` في `CORS_ORIGINS` **خاطئ** — `backend/.env:24` يحتوي فعلاً `financial-ssot.preview.emergentagent.com` + `car-repair-sys.emergent.host` + `alkobir.com`/`www` + `fixsa.online`/`www` + `localhost:3000`. **DRIFT = NONE**. لم يُمَس أي `.env`.
- **متبقٍ (P1، مؤجَّل بأمر المالك)**: `P1-SEC-UPLOAD` — 5 مسارات رفع على قرص العُقدة (`server.py:2918` · `routes_vehicle_files.py:38` · `routes_document_templates.py:330` · `routes_finance_bot.py:573` · `routes_references.py:142`). وبند جديد مُكتشَف: RBAC/تدقيق على مسارات reset هدّامة داخل FastAPI (سلوك قائم سابقاً، غير مُستحدث) — `routes_finance.py:5668,5672,5684-5685` · `routes_extended.py:2585`.
- تقرير التحقق: `/app/test_reports/iteration_379.json`.


## جلسة 2026-06 (فرع جديد) — PRODUCTION SUPABASE RUNTIME VERIFICATION — READ ONLY ✅ (بلا نشر/بلا تعديل أسرار/بلا تعديل بيانات)
- **الهدف**: إثبات project ref للإنتاج + مصدر SUPABASE_URL + مصدر مفتاح السيرفر + نوع المفتاح الفعلي، قراءة فقط.
- **PROJECT_REF_MATCH = YES** — `kqjlyozhvwswooztccag` هو ref مشروع `beeeta8`. **تصحيح رسمي** للتقرير السابق `SUPABASE_PROJECT_MATCH = NO` (كان يقارن اسم المشروع بالـref).
- **مصدر إعدادات المعاينة مثبت مباشرة** من `/proc/<backend_pid>/environ`: المنصة **لا تحقن أي SUPABASE_\*** (24 متغيراً فقط، ولا قيمة `sb_secret_*`/`sb_publishable_*`)؛ supervisor يحقن `APP_URL` و`INTEGRATION_PROXY_URL` فقط ⇒ **PREVIEW_CONFIG_SOURCE = `backend/.env`**، والمفتاح فيه **LEGACY_JWT** (`sha256_8=28196474`, claims ref=kqjlyozhvwswooztccag role=service_role).
- **نسخة أرتيفاكت الإنتاج محددة بدقة**: حزمة واحدة `main.35a8f1ca.js` (متطابقة على `car-repair-sys.emergent.host` و`alkobir.com`). تحتوي `auth_session` (أُضيف 2026-08-19T23:23Z) وتفتقد `journal-card-creator-tag` (أُضيف 2026-08-20T21:55Z) وتحتوي fallback `credentials:"omit"` (أُزيل 2026-09-04T14:19Z) ⇒ **PRODUCTION_BUILD_COMMIT_WINDOW = 2026-08-19T23:23Z .. 2026-08-20T21:55Z**.
- **تأكيد مستقل من الـbackend (بصمة CORS runtime)**: الإنتاج يعيد `allow-methods: DELETE, GET, HEAD, OPTIONS, PATCH, POST, PUT` (= Starlette ALL_METHODS ⇒ إعداد wildcard) و`allow-headers` = صدى الطلب؛ بينما HEAD المحلي يعيد القوائم الصريحة ⇒ **PRODUCTION_BACKEND = PRE-b7c3a996**. الواجهة والخلفية في الإنتاج من نفس النافذة (متسقان).
- **النتيجة**: الإنتاج **لا يحتوي** Phase 1B Authorization Closure ولا Phase 1C CORS/Cookie ولا Same-Origin `/api` Transport. وحزمة الإنتاج تستخدم base **مطلق** `https://alkobir.com` (وليس `/api` نسبي) ⇒ من دومين `car-repair-sys.emergent.host` تصبح الطلبات cross-origin credentialed.
- **مصدر مفتاح الإنتاج** = `backend/.env` داخل الأرتيفاكت المنشور (كان **متتبَّعاً في git** حتى إزالته في 0ceb2ddc بتاريخ 2026-09-02، وقيمته في تلك النافذة LEGACY_JWT `sha256_8=28196474`). `server.py` يستخدم `load_dotenv(override=False)` فتسبق متغيرات المنصة الملف **فقط إن وُجدت** — ولم تُلاحظ أي حقن في هذه الفئة من العُقد.
- **PRODUCTION_SUPABASE_KEY_TYPE = NOT_VERIFIABLE** (لا يوجد endpoint يكشف project ref/نوع المفتاح؛ كل `/api/*` على الإنتاج = 401، و`/api/supabase/status` يعيد booleans فقط). **أقوى دليل ⇒ LEGACY**، لأن أرتيفاكت الإنتاج لم يتغير منذ 2026-08-20 فلم تصل قيمة `sb_secret_*` الجديدة من إعدادات المنصة.
- **LEGACY_JWT ما زال مقبولاً من المشروع** (قراءة حقيقية READ-ONLY: `vehicles select=id limit=1` ⇒ 206، `content-range=0-0/237`) ⇒ لا يمكن التمييز بين النوعين من الخارج.
- **Preview drift مسجَّل ولم يُصلَح** (بأمر المالك): `CORS_ORIGINS` في `backend/.env` يشير لـ`accounting-ssot-fix.preview...` بينما الـhost الحالي `financial-ssot.preview.emergentagent.com`؛ المسار الأساسي same-origin (`/api/health = 200`) غير متأثر ⇒ PREVIEW_CONFIGURATION_DRIFT (غير حاجب).
- **Mutation Guard**: صفر نشر، صفر تعديل `.env`، صفر كتابة DB، صفر محاولة دخول للإنتاج (تجنّب قفل brute-force). تعديلات lint غير مالية فقط: 11 `except:` مكشوف في `backend/tests/*` → `except Exception:`، و`insert_one(dict(doc))` في `routes_finance.py` + `routes_workshop_config.py` لمنع تسريب ObjectId.
- التقرير الكامل: `/app/memory/discovery/PRODUCTION_SUPABASE_RUNTIME_READONLY_VERIFICATION.md`.
...
- تم تحويل مسار مصادقة المتصفح إلى same-origin `/api` على host الحالي المدعوم `https://financial-ssot.preview.emergentagent.com`، بدون مزيد من CORS hacks وبدون تغيير JWT/Supabase/PIN/users/AccountingEngine/financial data/migrations/Phase2.
